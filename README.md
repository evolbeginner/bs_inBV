...
People have increasingly acknowledged the importance of using more complex mixtur substitution models in modeling sequence evolution, particularly when analysing deep phylogenies. `MCMCtree` supports various amino acid substitution models, LG+G+F being the most complex one. Nevertheless, these models do not include some frequently used ones in current phylogenetic analysis, such as +I, +R, mixture models like EX2, C10-C60, and their combinations, and branch length mixture model GHOST, among other. On the other side, most of not all of such more complex models, are implemented in the phylogenetic software `IQ-TREE`.
