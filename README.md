...
## Before starting
To generate `ref.tre`, you may want to run `MCMCtree` first using default settings. Then, run ` sed '4!d' in.BV > ref.tre `. Note that `ref.tre` is an unrooted tree that is read by MCMCtree for dating analysis. `bs_inBV` will reorder the tips of the tree generated by `IQ-Tree` according to that of `ref.tre`.
...
Wang, Sishuo, and Haiwei Luo. "Dating the bacterial tree of life based on ancient symbiosis." Systematic Biology (2025): 4, 639-655. ([link](https://academic.oup.com/sysbio/article/74/4/639/7976854?login=true))
